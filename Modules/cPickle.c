...
cpm_dumps(PyObject *self, PyObject *args, PyObject *kwds)
{
    ...
    if (!( PyArg_ParseTupleAndKeywords(args, kwds, "O|i:dumps", kwlist,     //1.解析参数
               &ob, &proto)))
        goto finally;

    if (!( file = PycStringIO->NewOutput(128)))     //2.新建buffer
        goto finally;

    if (!( pickler = newPicklerobject(file, proto)))        //3.新建Pickler
        goto finally;

    if (dump(pickler, ob) < 0)      //4.写buffer
        goto finally;

    res = PycStringIO->cgetvalue(file);     //转PyStr
